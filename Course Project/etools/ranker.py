...
train = pd.read_csv("etools/train.csv")
...
# The contents of a set of important e-mails.    
important = [
    NLP(clean(doc))
    for doc
    in train[train["Important"] == 1]["Content"]
]

# The contents of a set of unimportant e-mails.
unimportant = [
    NLP(clean(doc))
    for doc
    in train[train["Important"] == 0]["Content"]
]


def score(document: str, train: pd.DataFrame = None) -> float:
    """Assign an importance score to this document between 0 and 1.
    
    Args:
        document: An e-mail represented as a single string object.
        train: A DataFrame consisting of important e-mails. Defaults to the
            preloaded *important* set if this variable is unspecified.
    ...
    """
    train = important if train is None else train
    nlp   = NLP(document)
    return np.mean([nlp.similarity(doc) for doc in train])
